...
`reproduce.py` reproduces Table 2 exactly for German Credit and Bank Marketing. The Adult column's accuracy and group-fairness rows reproduce exactly; its QID rows and Figures 3–5 come from an earlier unseeded training run and are therefore close but not bit-for-bit identical (mean QID ~0.601 seeded vs. 0.619 reported). This is stated in `ARTIFACT.md`. Timing figures are hardware-dependent.
